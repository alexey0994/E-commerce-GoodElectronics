			...
			echo "
                        ...
                        <div class='col-md-4 col-xs-6'>
								<a href='product.php?p=$pro_id'><div class='product'>
									...
									<div class='add-to-cart'>
										<button pid='$pro_id' id='product' href='#' tabindex='0' class='add-to-cart-btn'><i class='fa fa-shopping-cart'></i> Добавить в корзину</button>
									</div>
								</div>
							</div>
			";
		...
		if($count > 0){
			echo "
				<div class='alert alert-warning'>
						...
						<b>Продукт уже был добавлен в корзин..!</b>
				</div>
			";//
		} else {
			...
			if (mysqli_num_rows($query) > 0) {
				echo "
					<div class='alert alert-warning'>
							<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
							<b>Продукт уже был добавлен в корзину</b>
					</div>";
					exit();
			}
			$sql = "INSERT INTO `cart`
			...
			if (mysqli_query($con,$sql)) {
				echo "
					<div class='alert alert-success'>
						<a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
						<b>Ваш продукт успешно добавлен!</b>
					</div>
				";
				exit();
			}
...
if (isset($_POST["count_item"])) {
	//Когда пользователь вошел в систему, мы считаем количество товара в корзине, используя id сеанса пользователя
	if (isset($_SESSION["uid"])) {
		$sql = "SELECT COUNT(*) AS count_item FROM cart WHERE user_id = $_SESSION[uid]";
	}else{
		//Когда пользователь не вошел в систему, мы считаем количество товара в корзине, используя уникальный IP-адрес пользователя
...
//Получить корзину из базы данных в выпадающее меню
if (isset($_POST["Common"])) {

	if (isset($_SESSION["uid"])) {
		//Когда пользователь вошел в систему, этот запрос будет выполнен
		...
		//Когда пользователь не вошел в систему, будет выполнен этот запрос
	...
	if (isset($_POST["getCartItem"])) {
		//показать элемент корзины в выпадающем меню
		if (mysqli_num_rows($query) > 0) {
			$n=0;
			$total_price=0;
            ...
            echo '<div class="cart-summary">
				    <small class="qty">'.$n.' Выбранные товар(ы)  </small>
				    <h5>'.$total_price.'₽</h5>
				</div>'
            ?>
    ...
    if (isset($_POST["checkOutDetails"])) {
		if (mysqli_num_rows($query) > 0) {
			//Отобразить элемент корзины пользователя с помощью кнопки «Готов к оформлению заказа», если пользователь не авторизован
			echo '<div class="main ">
			...
			<form method="post" action="login_form.php">
	               ...
	               <table id="cart" class="table table-hover table-condensed" id="">
    				<thead>
						<tr>
							...
							<th style="width:7%" class="text-center">Промежуточный итог</th>
				...
				while ($row=mysqli_fetch_array($query)) {
					...
					echo 
						...
						<tr>
							...
							<td data-th="Подитог" class="text-center"><input type="text" class="form-control total" value="'.$product_price.'₽" readonly="readonly"></td>
				...
				if (!isset($_SESSION["uid"])) {
					echo '
							...
							<a href="" data-toggle="modal" data-target="#Modal_login" class="btn btn-success">Оформление заказа</a></td>
								</tr>
							</tfoot>
					...
					echo '
						...
						<form action="checkout.php" method="post">
							...
							echo   
								'<input type="hidden" name="return" value="http://http://localhost/GoodElectronics/payment_success.php"/>
					                <input type="hidden" name="notify_url" value="http://localhost/GoodElectronics/payment_success.php">
									<input type="hidden" name="cancel_return" value="http://localhost/GoodElectronics/public_html/cancel.php"/>
									<input type="hidden" name="currency_code" value="USD"/>
									<input type="hidden" name="custom" value="'.$_SESSION["uid"].'"/>
									<input type="submit" id="submit" name="login_user_with_product" name="submit" class="btn btn-success" value="Оформление заказа">
						</form></td>
